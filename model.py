...
def transfered_inception():
    ...
    x = base_model.output
    #x = Flatten()(x)
    x = Dense(1024, activation="relu", kernel_regularizer = keras.regularizers.l2(0.003))(x)
    x = Dropout(0.5)(x)
    x = Dense(256, activation="relu", kernel_regularizer = keras.regularizers.l2(0.003))(x)
    predictions = Dense(25, activation="softmax", kernel_regularizer = keras.regularizers.l2(0.001))(x)
    base_model = keras.models.Model(base_model.input, predictions)
    base_model.compile(loss = 'categorical_crossentropy',  optimizer = Adam(lr = 0.0001, decay = 1e-6))
    return base_model
...
if __name__ == '__main__':
    ...
    model.summary()
    model.fit(X_train, Y_train, verbose = 2, epochs=100, batch_size=8, validation_split=0.1, 
            callbacks=[ModelCheckpoint('inception-transferlearning_model2.h5', monitor='val_loss', save_best_only=True)])
    model.save('model2.h5',  overwrite = True)
